...
public class ExcelIO {
    ...
    public static void initExcel(File file, String fileName, String[] colName) {
        format();
    ...
    public static void writeObjListToExcel(ArrayList<Integer>[] objList,
        ...
        if (objList[0] != null ) {
            ...
            try {
                ...
                for (int i = 0; i < 4; i++) {                                   //j行 i列
                    //ArrayList<String> list = (ArrayList<String>) objList.[i];
                    for (int j = 0; j < objList[i].size(); j++) {
                        sheet.addCell(new Label(i,j+1 , objList[i].get(j).toString(), arial12format));
                    }
                }
                writebook.write();
